    ...
    <aside class="max-w-62.5 ease-nav-brand z-990 fixed inset-y-0 my-4 ml-4 block w-full -translate-x-full flex-wrap items-center justify-between overflow-y-auto rounded-2xl border-0 bg-white p-0 antialiased shadow-none transition-transform duration-200 xl:left-0 xl:translate-x-0 xl:bg-transparent">
        ...
        <div class="items-center block w-auto max-h-screen overflow-auto h-sidebar grow basis-full">
            <ul class="flex flex-col pl-0 mb-0">
                ...
                <li class="mt-0.5 w-full">
                    <a class="py-2.7 shadow-soft-xl text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap rounded-lg bg-white px-4 font-semibold text-slate-700 transition-colors" href="<?= base_url() ?>profile">
                                    ...
                                    <g transform="translate(-1717.000000, -291.000000)" fill="#FFFFFF" fill-rule="nonzero">
                                        <g transform="translate(1716.000000, 291.000000)">
                                            <g transform="translate(1.000000, 0.000000)">
                                                <path class="" fill="white" d="M45,0 L26,0 C25.447,0 25,0.447 25,1 L25,20 C25,20.379 25.214,20.725 25.553,20.895 C25.694,20.965 25.848,21 26,21 C26.212,21 26.424,20.933 26.6,20.8 L34.333,15 L45,15 C45.553,15 46,14.553 46,14 L46,1 C46,0.447 45.553,0 45,0 Z">
                                                </path>
                                                <path class="" fill="white" d="M22.883,32.86 C20.761,32.012 17.324,31 13,31 C8.676,31 5.239,32.012 3.116,32.86 C1.224,33.619 0,35.438 0,37.494 L0,41 C0,41.553 0.447,42 1,42 L25,42 C25.553,42 26,41.553 26,41 L26,37.494 C26,35.438 24.776,33.619 22.883,32.86 Z">
                                                </path>
                                                <path class="" fill="white" d="M13,28 C17.432,28 21,22.529 21,18 C21,13.589 17.411,10 13,10 C8.589,10 5,13.589 5,18 C5,22.529 8.568,28 13,28 Z">
                                                </path>
                                            </g>
                                        </g>
                                    </g>
    ...
    <main class="ease-soft-in-out xl:ml-68.5 relative h-full max-h-screen rounded-xl transition-all duration-200">
        ...
        <div class="px-10 py-2">
            <div class="flex flex-col items-center sm:flex-row">
                <div class="avatar mb-5 sm:mb-0 sm:mr-10">
                    <div class="w-24 sm:w-40 rounded">
                        <img src="https://api.dicebear.com/8.x/initials/svg?seed=<?= $session->get('user_name'); ?>" />
                    </div>
                </div>
                <div class="">
                    <div class="">
                        <h2 class="font-bold text-2xl"><?= $session->get('user_name'); ?></h2>
                    </div>
                    <div class="flex flex-row items-center gap-2">
                        <h2 class="text-lg"><?= $session->get('user_job'); ?></h2>
                        <!-- <div class=""><a href=""><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
                    ...
                    <div class="flex flex-row items-center gap-2">
                        ...
                        <h2 class="font-bold"><?= $session->get('user_email'); ?></h2>
                    </div>
                    <div class="flex flex-row items-center gap-2">
                        <p class="text-slate-400">Password:</p>
                        </p>
                        <h2 class="font-bold"><?= $session->get('user_password'); ?>********</h2>
                    </div>
                    <button data-twe-toggle="modal" data-twe-target="#exampleModal" class="mt-4 px-8 py-2 font-bold text-center uppercase align-middle transition-all bg-transparent border border-solid rounded-lg shadow-none cursor-pointer active:opacity-85 leading-pro text-xs ease-soft-in tracking-tight-soft bg-150 bg-x-25 hover:scale-102 active:shadow-soft-xs border-sky-500 text-sky-500 hover:text-sky-500 hover:opacity-75 hover:shadow-none active:scale-100 active:border-sky-500 active:bg-sky-500 active:text-white hover:active:border-sky-500 hover:active:bg-transparent hover:active:text-sky-500 hover:active:opacity-75">Edit</button>
                </div>
            </div>
        </div>
                ...
                <div class="relative w-auto m-2 transition-transform duration-300 pointer-events-none sm:m-7 sm:max-w-125 sm:mx-auto ease-soft-out -translate-y-13">
                    <div class="relative flex flex-col w-full bg-grey-100 border border-solid pointer-events-auto dark:bg-gray-950 bg-clip-padding border-black/20 rounded-xl outline-0">
                        <div class="flex items-center justify-between bg-white p-4 border-b border-solid shrink-0 border-slate-100 rounded-t-xl">
                            <h5 class="mb-0 leading-normal text-black" id="ModalLabel">Edit Email or Password</h5>
                            <svg xmlns="http://www.w3.org/2000/svg" class="ml-4" width="24" height="24" viewBox="0 0 24 24">
                                <path fill="currentColor" d="M20.71 7.04c.39-.39.39-1.04 0-1.41l-2.34-2.34c-.37-.39-1.02-.39-1.41 0l-1.84 1.83l3.75 3.75M3 17.25V21h3.75L17.81 9.93l-3.75-3.75z" />
                            </svg>
                        ...
                        <form action="<?= base_url('profile/updated') ?>" method="post">
                            ...
                            <div class="relative flex-auto p-4 bg-white">
                                <div class="flex flex-col">
                                    ...
                                    <input type="password" id="inputEditPassword" name="editpassword" class="py-2 px-3 bg-white">
                                </div>
                            </div>
                            <div class="flex flex-wrap items-center justify-end p-3 bg-white border-t border-solid shrink-0 border-slate-100 rounded-b-xl">
                                ...
                                <button type="submit" data-toggle="modal" data-target="#import" class="inline-block px-8 py-2 m-1 mb-4 text-xs font-bold text-center text-white uppercase align-middle transition-all border-0 rounded-lg cursor-pointer ease-soft-in leading-pro tracking-tight-soft bg-gradient-to-tl from-sky-700 to-indigo-500 shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85">Edit</button>
                            </div>
                        </form>
                    </div>
                </div>
